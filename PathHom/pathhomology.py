...
class PathHomology(object):
    ...
    def split_independent_digraph(all_components, edges):
        ...
        for i_c, component in enumerate(all_components):
            for i_e, edge in enumerate(edges):
                if (edges_visited[i_e] is False) and (
                    edge[0] in component or edge[1] in component
                ):
    ...
    def utils_generate_allowed_paths(self, edges, max_path):
        ...
        for i in range(0, max_path + 1):
            ...
            for path_previous in allowed_path[i]:
                for node in nodes:
                    if edge_matrix[
                        nodes_idx_map[path_previous[-1]], nodes_idx_map[node]
                    ]:
                        new_path = np.append(path_previous, node)
                        allowed_path[i + 1].append(new_path)
                        allowed_path_str[i + 1].append(
                            "->".join([str(one_node) for one_node in new_path])
                        )
                    else:
                        continue
    ...
    def utils_unlimited_boundary_operator(self, allowed_path, max_path):
        ...
        for n in range(1, max_path + 2):
            ...
            unlimited_boundary_mat = np.zeros(
                [allowed_path_n_types, len(considered_operated_path_name)]
            )
            for i_path, (n_1_path_str, operated_n_path_dict) in enumerate(
                boundary_map_dict.items()
            ):
            ...
            allowed_path_idx_argument[n] = [
                1 if tpn in allowed_path[n - 1] else 0
                for tpn in considered_operated_path_name
            ]
    ...
    def path_homology_for_connected_digraph(self, allowed_path, max_path):
        ...
        for n in range(1, max_path + 1):
            ...
            dim_An_Bn = np.linalg.matrix_rank(
                np.vstack(
                    [
                        np.eye(len(allowed_path_idx_argument[n + 1]))
                        * allowed_path_idx_argument[n + 1],
    ...
    def path_homology(self, edges, nodes, max_path):
        ...
        for i_d, edges in enumerate(all_digraphs):
            if np.shape(edges)[1] <= 1:
                ...
                all_path.append(allowed_path)
                betti_numbers.append(
                    self.path_homology_for_connected_digraph(allowed_path, max_path)
                )
        return np.sum(betti_numbers, axis=0), all_path

    def persistent_path_homology(
        self, cloudpoints, points_weight, max_path, filtration=None
    ):
        ...
        for n, snapshot_dis in enumerate(filtration):
            snapshot_map = (
                np.ones([points_num] * 2, dtype=int)
                * (distance_matrix <= snapshot_dis)
                * fully_connected_map
            )
    ...
    def persistent_angle_path_homology(
        self, cloudpoints, points_weight, max_path, filtration_angle_step=30
    ):
        ...
        for vz in range(filtration_angle_step, 180 + 1, filtration_angle_step):
            ...
            for vy in [0 * 180, 1 * 180]:
                ...
                for cross_vz_y in range(
                    filtration_angle_step, 180 + 1, filtration_angle_step
                ):
                    filtration.append([vz, cross_vz_y + vy])

        # initial vector
        initial_vector_x = np.array([0.0, 0.0, 0.0])
        max_distance = 0
        ...
        initial_vector_y = (
            cloudpoints[initial_vector_y_idx[0], :]
            - cloudpoints[initial_vector_y_idx[1], :]
        )
        ...
        initial_vector_y = np.cross(
            initial_vector_x, initial_vector_z
        )  # make sure for orthogonal
        ...
        for e_i, edge_v in enumerate(all_edge_vectors):
            ...
            edge_y_flag = (
                PathHomology().vector_angle(edge_v, self.initial_vector_y) // 90
            )
            ...
            snapshot_map_idx = np.append(
                ...
                np.where(
                    (
                        ...
                        * (
                            two_related_angles[:, 0]
                            > snapshot_angle[0] - filtration_angle_step
                        )
...
def parse_args(args):
    parser = argparse.ArgumentParser(
        description="Angle, distance, -based persistent path homology"
    )

    parser.add_argument(
        "--input_type", default="No", type=str, choices=["cloudpoints", "digraph", "No"]
    )
    parser.add_argument(
        "--input_data",
        default="cloudpoints.csv",
        type=str,
    ...
    parser.add_argument(
        "--filtration_type", default="angle", type=str, choices=["angle", "distance"]
    )
    parser.add_argument(
        "--max_distance",
        default=5,
        type=float,
        help="if filtration_type is angle, it will be ignored",
    )
    parser.add_argument(
        "--angle_step",
        default=30,
        type=int,
        help="Int, divisible by 180. if filtration_type is distance, it will be ignored",
    )
